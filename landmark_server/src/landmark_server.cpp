...
LandmarkServer::LandmarkServer() : num_frame_(0), num_landmark_(0) {}
...
bool LandmarkServer::AddNewFeatureAssociationToLastFrame(
  ...
  for (int i = 0; i < matches.size(); ++i) {
    ...
    if (feature_in_landmark_id_.back()[pre_id] != -1) {
      ...
      new_feature_in_landmark_id[new_id] =
          feature_in_landmark_id_.back()[pre_id];
      // map landmark to a feature in the latest frame
      landmark_in_camera_id_[new_feature_in_landmark_id[new_id]].back() =
          new_id;
    } else {
      // The feature is not in the landmark set, add
      vector<int> new_landmark_to_feature(feature_pos_in_camera_.size() - 2,
                                          -1);
...
bool LandmarkServer::MakeFeatureVectorsForReconstruct(
  ...
  for (int frame_id = 0; frame_id < num_frame_; ++frame_id) {
    for (int landmark_id = 0; landmark_id < num_landmark_; ++landmark_id) {
      if (landmark_in_camera_id_[landmark_id][frame_id] != -1) {
        ...
        feature_vectors[frame_id].push_back(
            feature_pos_in_camera_[frame_id][feature_id]);
      } else {
...
bool LandmarkServer::PrintStats() {
  ...
  for (int i = 0; i < num_landmark_; ++i) {
    for (int j = 0; j < num_frame_; ++j)
      if (landmark_in_camera_id_[i][j] != -1) seen_count[i]++;
  }
  for (int count = num_frame_; count >= 1; --count) {
    int temp_count = 0;
    for (int i = 0; i < num_landmark_; ++i)
      if (seen_count[i] == count) temp_count++;
